...
SELECT
    ...
    if(
        ...
        toString(
            least(
                hire_date + toIntervalDay(
                    30 + (cityHash64(employee_num * 23) % 335)
                ),
                today()
            )
        ),
...
SELECT
    ...
    if(
        ...
        concat('Employee ', toString(allocation_num))
    ) AS EmpName,
    p.position_id AS pos_id,
    multiIf(
        cityHash64(allocation_num * 3) % 10 < 3,
        lower(p.position_title),
        cityHash64(allocation_num * 3) % 10 < 6,
    ...
    if(
        ...
        toString(
            least(
                start_dt + toIntervalDay(
                    30 + cityHash64(allocation_num * 19) % 335
                ),
                today()
            )
        ),
...
FROM allocations a
JOIN positions p
    ON (cityHash64(allocation_num * 7) % 20) = p.pos_idx
CROSS JOIN (
    SELECT
        toDate('2023-01-01')
        + toIntervalDay(
...
**app/api/entities/route.ts** — Entity API route — Next.js API endpoint for CRUD operations on entities. Reads/writes to ClickHouse gold schema via HTTP interface.
...
|---------|------|---------|
| ClickHouse | 9000 | Native protocol (Dagster, dbt, change requests) |
| ClickHouse | 8123 | HTTP interface (Rill, Portal) |
